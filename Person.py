...
class Person:
    def __init__(self, canvas, age, home, status, position, still_working, icu_worker, edge_size, local_market, local_icu, quad, quad_i, dim):
        ...
        self.still_working = still_working        #bools of whether a person is still working, and whether he is an icu worker
        self.icu_worker = icu_worker
        self.time_sick = 0
        self.time_to_last_market = 0
        self.shape = canvas.create_oval(self.position[0] * scale + shift, self.position[1] * scale + shift,          #creating tkinter object for person
                                        self.position[0] * scale + size + shift,
                                        self.position[1] * scale + size + shift, fill='blue')

    def move(self, speed, work_increase_in_chance):
        if(self.status == "Quarantined"):                      #quarantined people stay home - people who don't follow quarantine are never assigned the quarantined status
            return
        ...
        if(self.icu_worker == True):                           #to simulate long shifts at icu, we rotate them between home and icu every day
            if self.position == self.local_icu:
                ...
                self.canvas.move(self.shape, self.home[0] * scale - self.local_icu[0] * scale,           #along with moving a person, we also have to move their object on screen
        ...
        if(self.still_working):                                                       #working people will go in a random direction, but more likely to head back home (gaussian distr in direc func)
            direc = self.direct()

            self.position[0] = self.position[0] + speed * work_increase_in_chance * direc[0]       #work incr in chance is represented as a faster speed for workers bc of how our model simulates interactions
        ...
        if (np.sqrt((self.position[0] - self.home[0]) ** 2 + (self.position[1] - self.home[1]) ** 2) > 0.5):      #even with direction more likely to be home, we still send people home if they stray too far
    ...
    def direct(self):                                  #generating a random direction for someone to go to, with home being the peak of the random distribution of directions (gaussian)
        home_direct=[0,0]
    ...
    def change_in_status (self, parent, people_list, chance_know_sick, perc_obey):           #checking grid cell it is on and thos above, below, left and right for infected people and calculating probability of infection. If not healthy person, see their status

        if self.status == "Healthy":
            count = 0
            pos = self.position
            for p in self.quad[self.quad_i]:              #count infected, quarantined and hospitalized people person comes into contact with

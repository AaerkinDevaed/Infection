...
class Person:
    ...
    def move(self, speed, work_increase_in_chance):
        ...
        if ((self.canvas.coords((self.shape))[0] - self.home[0]*scale)**2 + (self.canvas.coords((self.shape))[1] - self.home[1]*scale)**2 > 100):
            # even with direction more likely to be home, we still send people home if they stray too far
            self.canvas.delete(self.shape)
            if(self.status == "Healthy"):
                self.shape = self.canvas.create_oval((self.home[0] + self.city_loc[0]) * scale + shift,
                                            (self.home[1] + self.city_loc[1]) * scale + shift,
                                            # creating tkinter object for person
                                            (self.home[0] + self.city_loc[0]) * scale + size + shift,
                                            (self.home[1] + self.city_loc[1]) * scale + size + shift, fill='blue')
            elif (self.status == "Quarantined"):
                self.shape = self.canvas.create_oval((self.home[0] + self.city_loc[0]) * scale + shift,
                                                     (self.home[1] + self.city_loc[1]) * scale + shift,
                                                     # creating tkinter object for person
                                                     (self.home[0] + self.city_loc[0]) * scale + size + shift,
                                                     (self.home[1] + self.city_loc[1]) * scale + size + shift,
                                                     fill='yellow')
                ...
                self.shape = self.canvas.create_oval((self.home[0] + self.city_loc[0]) * scale + shift,
                                                     ...
                                                     (self.home[1] + self.city_loc[1]) * scale + size + shift,
                                                     fill='grey')
            elif (self.status == "Infected" or self.status == "Newly Infected"):
                self.shape = self.canvas.create_oval((self.home[0] + self.city_loc[0]) * scale + shift,
                                                     (self.home[1] + self.city_loc[1]) * scale + shift,
                                                     # creating tkinter object for person
                                                     (self.home[0] + self.city_loc[0]) * scale + size + shift,
            ...
            #self.canvas.move(self.shape, (self.home[0] * scale, self.home[1] * scale))
            return
    ...
    def direct(
        ...
        sp = [random.gauss(home_direct[0], 0.75), random.gauss(home_direct[1], 0.75)]  # random dir with home as peak of gauss
    ...
    def change_in_status(self, parent, people_list, chance_know_sick,
        ...
        if self.status == "Healthy":
            ...
            self.time_sick += 1
            if random.random() < perc_obey * chance_know_sick * strength:  # a portion of sick people know they're sick, and a portion of those will behave morally and self-isolate

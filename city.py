...
class City:
    def __init__(self, canvas, population, pop_density, city_type, city_name, city_loc):
        ...
        for x in range(self.homes):
            ...
            for p in range(5):
                ...
                p = Person(self, self.canvas, age, home, status, position, still_working, icu_worker, self.side_length,
    ...
    def travel_complete(self, p):
        ...
        temp = swappee.age, swappee.status  # a simple way for the new person to "become a resident is to swap status, age, etc. with a similar resident of target city
        swappee.swap_change([p.age, p.status])
        p.swap_change(temp)
        p.go_back()
        return
    ...
    def next_day(self):
        ...
        if self.num_infected > 10:                       #first 5 days no travel to start infections up
            target = choice(self.neighbors)
            temp = True
            status = ""
            while temp == True and status != "Quarantined":  # going to assume working people (and therefore icu workers) don't travel
